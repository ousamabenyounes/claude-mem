...
/**
 * Build the static system prompt from mode configuration.
 *
 * Contains all mode-specific instructions that do not change between calls:
 * identity, role, recording guidance, output format, and XML schema.
 * Intended for the SDK's systemPrompt option so the content is cacheable
 * and not re-sent as a user message on every turn.
 *
 * Issue #1891: Previously these instructions were concatenated into every
 * user message, wasting tokens and preventing prompt caching.
 */
...
/**
 * Build initial user message for SDK agent (dynamic content only).
 *
 * Used together with buildSystemPrompt() when the caller supports a
 * separate system prompt channel (e.g. the Claude Agent SDK's systemPrompt
 * option). Contains only the per-request dynamic content: the user's
 * request with timestamp and the session start header.
 */
export function buildInitUserMessage(userPrompt: string, mode: ModeConfig): string {
...
/**
 * Build continuation user message for SDK agent (dynamic content only).
 *
 * Used together with buildSystemPrompt() when the caller supports a
 * separate system prompt channel. Contains only the per-request dynamic
 * content: continuation greeting, user request, continuation instruction,
 * and the session continuation header.
 */
export function buildContinuationUserMessage(userPrompt: string, mode: ModeConfig): string {
 ...
 *
 * NOTE: This is the legacy all-in-one prompt used by agents that do not
 * support a separate system prompt (GeminiAgent, OpenRouterAgent).
 * For the Claude Agent SDK, prefer buildSystemPrompt() + buildInitUserMessage().
 */
 ...
 * NOTE: This is the legacy all-in-one prompt used by agents that do not
 * support a separate system prompt (GeminiAgent, OpenRouterAgent).
 * For the Claude Agent SDK, prefer buildSystemPrompt() + buildContinuationUserMessage().
 *
